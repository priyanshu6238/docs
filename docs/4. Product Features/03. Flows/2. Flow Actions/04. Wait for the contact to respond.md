...
All text matching is case-insensitive, and leading or trailing spaces in the reply are ignored. When a criteria contains several words, separate them with commas or spaces.
...
Accepts the input if it contains **all** of the words defined in the response criteria. The words can appear in any order, and the reply may contain extra words. Each word has to appear in full — it is not matched as part of a longer word.

Example: Criteria = `one, two`
- `one two` ✅
- `two one` ✅ (order does not matter)
- `one two three` ✅ (extra words are allowed)
- `one` ❌ (`two` is missing)
- `one1` ❌ (`one` has to be a word on its own)

Note that only commas, semicolons, and spaces separate words in the reply. Other punctuation stays attached to the word next to it, so a reply of `one, two!` would not match the criteria above.

### 3. has a phrase
Accepts the input if the criteria appears **anywhere inside** the reply, as an unbroken sequence of words in that exact order. Anything before or after the phrase is ignored.

Example: Criteria = `good morning`
- `good morning, how are you?` ✅
- `Yes, good morning` ✅ (text before the phrase is fine)
- `morning, good sir` ❌ (the words are out of order)
- `good day` ❌ (the phrase is not present)

Note that this is a plain text search rather than a whole-word match, so a criteria of `art` would also accept a reply of `start`.

### 4. has only the phrase
Accepts the input only if the **whole reply** matches the criteria exactly, with nothing before or after it. Letter case and surrounding spaces are ignored; any other extra character makes the reply invalid.

Example: Criteria = `good morning`
- `good morning` ✅
- `Good Morning` ✅ (case is ignored)
