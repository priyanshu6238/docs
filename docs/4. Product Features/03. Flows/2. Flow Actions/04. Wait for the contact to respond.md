...
Accepts the input if it contains all of the words defined in the response criteria, in any order. Multiple words can be added, separated by commas. Extra words in the reply are fine, as long as every word in the criteria is present. Matching is on whole words and is not case sensitive.
...
### 3. has a phrase
Accepts the input if the phrase defined in the response criteria appears anywhere in the response, as the same words in the same order. Matching is not case sensitive.
Example: Criteria = good morning, so "good morning, how are you?" and "Yes. Good morning" are accepted, while "morning, good sir" is not (the words are out of order).

### 4. has only the phrase
Accepts the input only if the whole response matches the response criteria exactly, with nothing before or after it. Leading and trailing spaces are ignored, and matching is not case sensitive.
