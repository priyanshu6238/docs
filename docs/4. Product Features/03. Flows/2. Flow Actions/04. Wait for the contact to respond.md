...
Accepts the input if it contains **all** of the words defined in the response criteria, in any order. Multiple words can be added, separated by commas. Extra words in the reply are fine — every word in the criteria just has to be present somewhere in it.
Example: Criteria = `red, apple`, so `I want a red apple` and `apple, red` are accepted, while `I want a red banana` is not (the word `apple` is missing).

Matching is case-insensitive and works on whole words, so `red apples` does not count as the word `apple`, and punctuation stuck to a word (for example `apple.`) will stop it from matching.

### 3. has a phrase
Accepts the input if the exact phrase — the same words, in the same order — appears anywhere in the reply. Anything before or after the phrase is ignored, and matching is case-insensitive.
Example: Criteria = `good morning`, so `good morning` and `Yes, good morning, how are you?` are accepted, while `morning, good sir` is not (the words are not in that order).

### 4. has only the phrase
Accepts the input only if the whole reply is exactly what is defined in the response criteria, with nothing before or after it. Leading and trailing spaces are ignored and matching is case-insensitive, but any other extra character counts.
Example: Criteria = `good morning`, so `good morning` and `Good Morning` are accepted, while `good morning!` and `good morning to you` are not.
