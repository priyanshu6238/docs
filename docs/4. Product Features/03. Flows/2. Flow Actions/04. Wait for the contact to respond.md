...
Accepts the input only if **every** word listed in the criteria appears in the response, in **any order**. Extra words in the response are allowed. Words in the criteria can be separated by commas or spaces.

Matching is case-insensitive and works on whole words, so a partial word does not count.

For example, if the criteria is `red, apple`:
- `I want a red apple` ✅
- `apple red` ✅ (order does not matter)
- `I want a red banana` ❌ (missing `apple`)
- `reddish apple` ❌ (`reddish` is not the whole word `red`)

### 3. has a phrase
Accepts the input if the criteria appears **anywhere inside** the response as the same sequence of words. Anything before or after the phrase is allowed, but the words must appear in that exact order. Matching is case-insensitive.

For example, if the criteria is `good morning`:
- `good morning` ✅
- `good morning, how are you?` ✅
- `Hi, good morning` ✅
- `morning, good sir` ❌ (words are out of order)

### 4. has only the phrase
Accepts the input only if the **whole response** is exactly the criteria, with nothing before or after it. Upper/lower case and extra spaces at the start or end are ignored, but any additional word or punctuation makes the response invalid.
...
### Choosing between the text options

| Option | Order matters? | Extra words allowed? |
| --- | --- | --- |
| has any of the words | No | Yes — only one of the words is needed |
| has all of the words | No | Yes — but every word in the criteria must be present |
| has a phrase | Yes | Yes — before or after the phrase |
| has only the phrase | Yes | No — the response must be the phrase and nothing else |
