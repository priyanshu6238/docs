...
Accepts the input only if it contains **all** of the words defined in the response criteria. The words can appear in any order, and extra words in the reply are ignored. Multiple words can be added, separated by commas.

Example: Criteria = `red, apple`
- `I want a red apple` ✅
- `apple red` ✅ (order does not matter)
- `I want a red banana` ❌ (missing `apple`)

Matching is case-insensitive, and each word has to appear as a whole word — the reply is split on spaces, commas and semicolons, so a word that carries other punctuation (for example `apple!`) is not treated as a match for `apple`.

### 3. has a phrase
Accepts the input if the phrase defined in the criteria appears anywhere in the reply, as a continuous sequence of words in that same order. Anything typed before or after the phrase is ignored.

Example: Criteria = `good morning`
- `good morning` ✅
- `good morning, how are you?` ✅
- `Yes, good morning` ✅
- `morning, good sir` ❌ (the words are not in the same order)

### 4. has only the phrase
Accepts the input only if the whole reply matches the criteria exactly, with nothing before or after it. Extra spaces at the start or end and differences in letter case are ignored, but any additional word or punctuation makes the response invalid.

Example: Criteria = `good morning`
- `good morning` ✅
- `Good Morning` ✅ (matching is case-insensitive)
- `good morning!` ❌ (extra punctuation)
- `good morning to you` ❌ (extra words)

### Choosing between the text criteria
| Criteria | Matches when the reply… | Criteria = `good morning` |
| --- | --- | --- |
| has any of the words | contains at least one of the listed words | `good day` ✅ |
| has all of the words | contains every listed word, in any order | `morning, good sir` ✅ |
| has a phrase | contains the words together, in the given order | `good morning, how are you?` ✅ |
| has only the phrase | is exactly the criteria and nothing else | `good morning` ✅ |
