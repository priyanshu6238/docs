...
Accepts the input if it contains **all** of the words defined in the response criteria, in any order. Extra words in the reply are fine — the reply only has to include every word from the criteria.
Example: Criteria = `red, apple`
- "I want a red apple" — accepted.
- "apple red" — accepted (order does not matter).
- "I want a red banana" — not accepted ("apple" is missing).

### 3. has a phrase
Accepts the input if the phrase defined in the response criteria appears anywhere inside the reply, as the same sequence of characters and in the same order.
Example: Criteria = `good morning`
- "good morning, how are you?" — accepted.
- "Yes. Good morning!" — accepted (the phrase appears within the reply).
- "morning, good sir" — not accepted (the words are out of order).

### 4. has only the phrase
Accepts the input only if the whole reply matches the response criteria exactly, with nothing else before or after it.
Example: Criteria = `good morning`
- "good morning" — accepted.
- "good morning!" — not accepted (the extra `!` makes it a different reply).
- "good morning to you" — not accepted (extra words).

> **Notes on matching**
> - All of these options ignore capitalisation, so `Yes`, `yes`, and `YES` are treated the same.
> - `has any of the words` and `has all of the words` compare whole words, and treat commas, semicolons, and spaces as word separators. Other punctuation stuck to a word stays part of it, so a reply of "apple." will not match the word `apple` — prefer `has a phrase` when the reply is likely to contain punctuation.
